...
notes.innerHTML += `<div class="noteCard card" id=${itemCount} style="width: 15rem;  
height:17rem; background-color:white; color:black; margin-right:10px; margin-bottom:20px">
<center>
    <span style="font-size:18px" class="card-text">${cardTitle}</span>
</center>    
    <hr>
   <ul class="tasks" id="itemList${itemCount}" style="height:150px; border: 2px solid red;font-size:10px "></ul><i class="fa-solid fa-plus" onclick="popup2(); idList(event)" id="add_btn_${itemCount++}" style="cursor: pointer;float:right;margin-top:5px;margin-right:5px;background-color:blue;border-radius:50%;color:white;font-size:25px "></i>
    <i  onclick="deleteNote(this) " class="fa-solid fa-trash-can dltBtn" style="cursor: pointer;float:right;margin-top:5px;margin-right:5px;border-radius:50%;font-size:25px;color:red; "></i>
</div>`
}
...
let addItem=()=>{
    ...
    li.innerHTML=`<span>${addTxt2}</span><button id="markDone" onclick="cutIfWorkDone(this)">Markdone</button>`
}
